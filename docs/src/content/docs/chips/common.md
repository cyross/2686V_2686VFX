...
3.4.0 から、**FM 音源で作った音色を OPZX7S の音色へ直して持っていけます。**
ボタンは 2 つあり、どちらも同じ直し方をします。

| ボタン | 置き場所 | はたらき |
| --- | --- | --- |
| **[EX]OPZX7S Params** | OPNA / OPN / OPL / OPL3 / OPM の UTILITY | いまのチャンネルを、OPZX7S の CH Params ファイルとして書き出します |
| **[IM]FM Params** | OPZX7S の UTILITY | OPNA / OPN / OPL / OPL3 / OPM の CH Params ファイルを選び、OPZX7S の値へ直して読み込みます |

[EX]OPZX7S Params は 2686V・2686VLight・26V・86V・OPNV・OPLV・OPMV に、
[IM]FM Params は 2686V・2686VLight・OPZX7S にあります。**OPZX7S プラグインは
FM 音源を持っていませんが、ほかのプラグインで保存したファイルを読めます。**

[IM]FM Params で読み込むと、読み込んだことを知らせるダイアログが出ます。
閉じると保存先を選ぶ画面が開き、**そのまま OPZX7S の CH Params として残せます。**
名前は元のファイルから付けます（`piano.param.opm.json` なら
`piano.param.opzx7s.json`）。残さないときは取り消してください。

#### 直し方

| 元の音源 | OPZX7S |
| --- | --- |
| アルゴリズム | **アルゴリズムマトリックス**へ同じつなぎ方を写します |
| FB | 戻りを受けるオペレーターの FB へ、同じ深さになる値で入れます |
| AR / DR (D1R) / SR (D2R) / RR | **実数モード**（Register Mode を切）の秒へ、実機のレート表で直します |
| SL (D1L) / TL | 実数モードのレベルへ直します（SL は 3dB、TL は 0.75dB 刻み） |
| EG TYPE が切 (OPL / OPL3) | 減衰音。SL へ達したあと、RR の速さで落ち続けるように SR を決めます |
| SUS (OPL) | 離したあとをレート 5 で落とすので、その速さを RR へ入れます |
| MUL / MUL.RATIO | 同じ倍率の MUL へ。OPM の MUL.RATIO はそのまま使います |
| DT1 / DT2 | DT1 は同じ量のセントで DT3 へ、DT2 は DT2 へ |
| KS / KSR / KSL | OPN と OPM は OPZ の、OPL と OPL3 は MA-7 のキースケールへ（刻みは同じ） |
| 波形 (OPL / OPL3) | 同じ名前の WAVE SHAPE へ。OPN 系と OPM は Sine |
| SSG-EG (OPNA) | そのまま写します |
| OPNA のハードウェア LFO | オペレーターの LFO へ、同じ速さ・深さの三角波で |
| N88 LFO (OPNA / OPN) | PM はチャンネル全体の LFO へ、AM は N88 AMS の深さでオペレーターの LFO へ |
| OPM の LFO | PM はチャンネル全体の LFO へ、AM は AMS-EN を入れたオペレーターの LFO へ |
| AM / VIB (OPL / OPL3) | オペレーターの LFO へ |
| PAN (OPNA / OPM) | 左右どちらかなら OPZX7S のパンの端へ。中央ならパンを切ります |

LEVEL・AMP ENV・SSG HW ENV・UNISON・QUALITY・オペレーターの PITCH ENV など、
**どの音源も同じ部品を使っている区分は、そのまま写します。**

元の音源に無いオペレーター（OPL なら 3〜8、ほかは 5〜8）は、**どこにも
つながず**、OPZX7S の初期値を入れます。音には出ません。

#### 近い値へ寄せるもの

同じ動きにできない項目は、近い値へ寄せます。**寄せたものがあったときは、
最後にダイアログで一覧を出します。**

- DT1 は OPZX7S の刻みと合わないので、1 セント単位に丸めて DT3 へ入れます
- 実数モードのキースケール（KSR）は、時間の縮め方がレジスタとは違います
- 10 秒を超える段は 10 秒にします。SR (D2R) が 10 秒を超えるときは減衰しない扱いにします
- AR が 0（立ち上がらない）のオペレーターは、AR を最長の 10 秒にします
- OPZX7S で表せないほど浅い FB は、表せる一番浅い値にします
- 左右どちらかだけのパンは、OPZX7S のパンの端にします。端でも反対側がわずかに鳴ります
- LFO の速さと深さは、OPZX7S の範囲へ収めます
- N88 LFO の PMD / AMD が負（逆向きの揺れ）のものは、向きを戻します
- HW LFO と N88 LFO の AM を両方使っているオペレーターは、HW LFO の AM だけを使います

:::note
- 3.0.0 より前の**行並びの形式**のファイルは、[IM]FM Params では読めません
- 読み込むとき、元の音源に無いオペレーターの PITCH ENV などの区分は、
  読み込む前の値のまま残ります。どこにもつながないので音には出ません
:::
