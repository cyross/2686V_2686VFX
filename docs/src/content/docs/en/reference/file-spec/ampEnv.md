...
description: What is inside .ampEnv.json
sidebar:
  order: 1
---
...
| | |
| --- | --- |
| **Extension** | `.ampEnv.json` / `.ampEnv.yaml` |
| **format** | `ampEnv` |
| **version** | `1` |
| **Export / import** | AMP ENV's [EX] / [IM] |
...
`ampEnv` sits under `values`, and the keys are inside it.
