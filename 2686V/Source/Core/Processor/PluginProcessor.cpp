...
SynthParams AudioPlugin2686V::buildRenderParams()
{
    ...
    if (params.mode == OscMode::OPZX7)
    {
        params.opzx7.algFb.matrix.mode = getOpzx7AlgMode();

        const AlgMatrixState& guiState = getOpzx7AlgMatrixForAudio();

        for (int i = 0; i < 8; ++i) {
            params.opzx7.algFb.matrix.isCarrier[i] = guiState.isCarrier[i];

            for (int j = 0; j < 8; ++j) {
                params.opzx7.algFb.matrix.mod[i][j] = guiState.mod[i][j];
                params.opzx7.algFb.matrix.fbMod[i][j] = guiState.fbMod[i][j];
            }
        }
    }
...
void AudioPlugin2686V::prepareRenderVoice(SynthVoice& voice, double sampleRate)
{
    ...
    {
        if (opzx7PcmBuffers[op].empty()) voice.clearOpzx7PcmBuffer(op);
        else voice.setOpzx7PcmBuffer(op, &opzx7PcmBuffers[op]);

        if (opzx7WtBuffers[op].empty()) voice.clearOpzx7WtBuffer(op);
        else voice.setOpzx7WtBuffer(op, &opzx7WtBuffers[op]);
