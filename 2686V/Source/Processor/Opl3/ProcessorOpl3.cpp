...
void Opl3Processor::createLayout(juce::AudioProcessorValueTreeState::ParameterLayout& layout)
{
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>(prefix + Opl3PrKey::level, prefixName + Opl3PrName::level, Opl3PrValue::Level::min, Opl3PrValue::Level::max, Opl3PrValue::Level::initial));

    // ==============================================================================
    // OPL3 Parameters (4 Ops, 8 Waveforms)
    // ==============================================================================
    layout.add(std::make_unique<juce::AudioParameterInt>(prefix + Opl3PrKey::alg, prefixName + Opl3PrName::alg, Opl3PrValue::Alg::min, Opl3PrValue::Alg::max, Opl3PrValue::Alg::initial));
    layout.add(std::make_unique<juce::AudioParameterInt>(prefix + Opl3PrKey::fb, prefixName + Opl3PrName::fb, Opl3PrValue::Fb::min, Opl3PrValue::Fb::max, Opl3PrValue::Fb::initial));
    layout.add(std::make_unique<juce::AudioParameterInt>(prefix + Opl3PrKey::bit, prefixName + Opl3PrName::bit, Opl3PrValue::Bit::min, Opl3PrValue::Bit::max, Opl3PrValue::Bit::initial));
    layout.add(std::make_unique<juce::AudioParameterInt>(prefix + Opl3PrKey::rate, prefixName + Opl3PrName::rate, Opl3PrValue::Rate::min, Opl3PrValue::Rate::max, Opl3PrValue::Rate::initial)); // Default 6 (16kHz)

    // ユニゾン・ハーモニー用
    layout.add(std::make_unique<juce::AudioParameterInt>(prefix + Opl3PrKey::Unison::voices, prefixName + Opl3PrName::Unison::voices, Opl3PrValue::Unison::Voices::min, Opl3PrValue::Unison::Voices::max, Opl3PrValue::Unison::Voices::initial));
    layout.add(std::make_unique<juce::AudioParameterInt>(prefix + Opl3PrKey::Unison::detune, prefixName + Opl3PrName::Unison::detune, Opl3PrValue::Unison::Detune::min, Opl3PrValue::Unison::Detune::max, Opl3PrValue::Unison::Detune::initial));
    layout.add(std::make_unique<juce::AudioParameterFloat>(prefix + Opl3PrKey::Unison::spread, prefixName + Opl3PrName::Unison::spread, Opl3PrValue::Unison::Spread::min, Opl3PrValue::Unison::Spread::max, Opl3PrValue::Unison::Spread::initial));

    for (int i = 0; i < Opl3PrValue::ops; ++i)
    {
        juce::String opPrefix = prefix + Opl3PrKey::op + juce::String(i);
        juce::String opPrefixName = prefixName + Opl3PrName::op + juce::String(i + 1);

        layout.add(std::make_unique<juce::AudioParameterInt>(opPrefix + Opl3PrKey::mul, opPrefixName + Opl3PrName::mul, Opl3PrValue::Op::Mul::min, Opl3PrValue::Op::Mul::max, Opl3PrValue::Op::Mul::initial));
        layout.add(std::make_unique<juce::AudioParameterBool>(opPrefix + Opl3PrKey::am, opPrefixName + Opl3PrName::am, Opl3PrValue::Op::Lfo::Am::initial)); // AM Enable (Switch)
        layout.add(std::make_unique<juce::AudioParameterBool>(opPrefix + Opl3PrKey::vib, opPrefixName + Opl3PrName::vib, Opl3PrValue::Op::Lfo::Vib::initial));
        layout.add(std::make_unique<juce::AudioParameterFloat>(opPrefix + Opl3PrKey::ams, opPrefixName + Opl3PrName::ams, Opl3PrValue::Op::Lfo::Ams::min, Opl3PrValue::Op::Lfo::Ams::max, Opl3PrValue::Op::Lfo::Ams::initial));
        layout.add(std::make_unique<juce::AudioParameterFloat>(opPrefix + Opl3PrKey::amd, opPrefixName + Opl3PrName::amd, Opl3PrValue::Op::Lfo::Amd::min, Opl3PrValue::Op::Lfo::Amd::max, Opl3PrValue::Op::Lfo::Amd::initial));
        layout.add(std::make_unique<juce::AudioParameterFloat>(opPrefix + Opl3PrKey::pms, opPrefixName + Opl3PrName::pms, Opl3PrValue::Op::Lfo::Pms::min, Opl3PrValue::Op::Lfo::Pms::max, Opl3PrValue::Op::Lfo::Pms::initial));
        layout.add(std::make_unique<juce::AudioParameterFloat>(opPrefix + Opl3PrKey::pmd, opPrefixName + Opl3PrName::pmd, Opl3PrValue::Op::Lfo::Pmd::min, Opl3PrValue::Op::Lfo::Pmd::max, Opl3PrValue::Op::Lfo::Pmd::initial));
        layout.add(std::make_unique<juce::AudioParameterBool>(opPrefix + Opl3PrKey::egType, opPrefixName + Opl3PrName::egType, Opl3PrValue::Op::EgType::initial)); // 1=Sustain, 0=Decay
        layout.add(std::make_unique<juce::AudioParameterBool>(opPrefix + Opl3PrKey::ksr, opPrefixName + Opl3PrName::ksr, Opl3PrValue::Op::Ksr::initial)); // 0=Low, 1=High
        layout.add(std::make_unique<juce::AudioParameterInt>(opPrefix + Opl3PrKey::ksl, opPrefixName + Opl3PrName::ksl, Opl3PrValue::Op::Ksl::min, Opl3PrValue::Op::Ksl::max, Opl3PrValue::Op::Ksl::initial));
        layout.add(std::make_unique<juce::AudioParameterInt>(opPrefix + Opl3PrKey::eg, opPrefixName + Opl3PrName::eg, Opl3PrValue::Op::Eg::min, Opl3PrValue::Op::Eg::max, Opl3PrValue::Op::Eg::initial));
        layout.add(std::make_unique<juce::AudioParameterBool>(opPrefix + Opl3PrKey::mask, opPrefixName + Opl3PrName::mask, Opl3PrValue::Op::Mask::initial)); // OP Mask (Switch)

        layout.add(std::make_unique<juce::AudioParameterBool>(opPrefix + Opl3PrKey::PitchAdsr::enable, opPrefixName + Opl3PrName::PitchAdsr::enable, Opl3PrValue::Op::PitchAdsr::Enable::initial));
        layout.add(std::make_unique<juce::AudioParameterBool>(opPrefix + Opl3PrKey::SsgSwEnv::enable, opPrefixName + Opl3PrName::SsgSwEnv::enable, Opl3PrValue::Op::SsgSwEnv::Enable::initial));

        layout.add(std::make_unique<juce::AudioParameterInt>(opPrefix + Opl3PrKey::rgAr, opPrefixName + Opl3PrName::rgAr, Opl3PrValue::Op::RgAdsr::Ar::min, Opl3PrValue::Op::RgAdsr::Ar::max, Opl3PrValue::Op::RgAdsr::Ar::initial));
        layout.add(std::make_unique<juce::AudioParameterInt>(opPrefix + Opl3PrKey::rgDr, opPrefixName + Opl3PrName::rgDr, Opl3PrValue::Op::RgAdsr::Dr::min, Opl3PrValue::Op::RgAdsr::Dr::max, Opl3PrValue::Op::RgAdsr::Dr::initial));
        layout.add(std::make_unique<juce::AudioParameterInt>(opPrefix + Opl3PrKey::rgSl, opPrefixName + Opl3PrName::rgSl, Opl3PrValue::Op::RgAdsr::Sl::min, Opl3PrValue::Op::RgAdsr::Sl::max, Opl3PrValue::Op::RgAdsr::Sl::initial));
        layout.add(std::make_unique<juce::AudioParameterInt>(opPrefix + Opl3PrKey::rgRr, opPrefixName + Opl3PrName::rgRr, Opl3PrValue::Op::RgAdsr::Rr::min, Opl3PrValue::Op::RgAdsr::Rr::max, Opl3PrValue::Op::RgAdsr::Rr::initial));
        layout.add(std::make_unique<juce::AudioParameterInt>(opPrefix + Opl3PrKey::rgTl, opPrefixName + Opl3PrName::rgTl, Opl3PrValue::Op::RgAdsr::Tl::min, Opl3PrValue::Op::RgAdsr::Tl::max, Opl3PrValue::Op::RgAdsr::Tl::initial));

        layout.add(std::make_unique<juce::AudioParameterBool>(opPrefix + Opl3PrKey::xof, opPrefixName + Opl3PrName::xof, Opl3PrValue::Op::Xof::initial)); // Xof (Switch)
        layout.add(std::make_unique<juce::AudioParameterBool>(opPrefix + Opl3PrKey::kor, opPrefixName + Opl3PrName::kor, Opl3PrValue::Op::Kor::initial)); // Kor (Switch)
        layout.add(std::make_unique<juce::AudioParameterBool>(opPrefix + Opl3PrKey::ampBypass, opPrefixName + Opl3PrName::ampBypass, Opl3PrValue::Op::AmpBypass::initial)); // Bypass (Switch)
...
void Opl3Processor::init(juce::AudioProcessorValueTreeState& apvts) {
    ...
    pLevel = apvts.getRawParameterValue(prefix + Opl3PrKey::level);

    pAlg = apvts.getRawParameterValue(prefix + Opl3PrKey::alg);
    pFb = apvts.getRawParameterValue(prefix + Opl3PrKey::fb);
    pDepth = apvts.getRawParameterValue(prefix + Opl3PrKey::bit);
    pRate = apvts.getRawParameterValue(prefix + Opl3PrKey::rate);

    pUnisonVoices = apvts.getRawParameterValue(prefix + Opl3PrKey::Unison::voices);
    pUnisonDetuneCents = apvts.getRawParameterValue(prefix + Opl3PrKey::Unison::detune);
    pUnisonSpread = apvts.getRawParameterValue(prefix + Opl3PrKey::Unison::spread);

    for (int op = 0; op < Opl3PrValue::ops; ++op)
    {
        juce::String p = prefix + Opl3PrKey::op + juce::String(op);
...
void Opl3Processor::processBlock(SynthParams& params, juce::AudioProcessorValueTreeState& apvts)
{
    ...
    params.opl3.feedback = (int)pFb->load(std::memory_order_relaxed);
