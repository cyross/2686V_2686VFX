...
void Opl3Processor::createLayout(juce::AudioProcessorValueTreeState::ParameterLayout& layout)
{
    ...
    for (int i = 0; i < PrValue::ops; ++i)
    {
        juce::String prefix = code + PrKey::op + juce::String(i);
        juce::String namePrefix = code + PrName::op + juce::String(i + 1);
...
void Opl3Processor::processBlock(SynthParams& params, juce::AudioProcessorValueTreeState& apvts)
{
    ...
    params.opl3.algorithm = (int)*apvts.getRawParameterValue(code + PrKey::alg);
    params.opl3.feedback = *apvts.getRawParameterValue(code + PrKey::fb0);
    params.opl3.feedback2 = *apvts.getRawParameterValue(code + PrKey::fb2);
    params.opl3.fmBitDepth = (int)*apvts.getRawParameterValue(code + PrKey::bit);
    params.opl3.fmRateIndex = (int)*apvts.getRawParameterValue(code + PrKey::rate);
    ...
    for (int op = 0; op < PrValue::ops; ++op)
    {
        ...
        params.opl3.op[op].multiple = (int)*apvts.getRawParameterValue(p + PrKey::mul);
        ...
        params.opl3.op[op].keyScaleLevel = (int)*apvts.getRawParameterValue(p + PrKey::ksl);
        ...
        params.opl3.op[op].waveSelect = (int)*apvts.getRawParameterValue(p + PrKey::eg);
        params.opl3.op[op].egType = *apvts.getRawParameterValue(p + PrKey::egType) > PrValue::boolThread;
        params.opl3.op[op].vibEnable = *apvts.getRawParameterValue(p + PrKey::vib) > PrValue::boolThread;
        params.opl3.op[op].pms = 0;
        params.opl3.op[op].amEnable = (*apvts.getRawParameterValue(p + PrKey::am) > PrValue::boolThread);
        params.opl3.op[op].ams = 0;
        params.opl3.op[op].oplAms = *apvts.getRawParameterValue(p + PrKey::ams);
        params.opl3.op[op].oplAmd = *apvts.getRawParameterValue(p + PrKey::amd);
        params.opl3.op[op].oplPms = *apvts.getRawParameterValue(p + PrKey::pms);
        params.opl3.op[op].oplPmd = *apvts.getRawParameterValue(p + PrKey::pmd);
        params.opl3.op[op].mask = (*apvts.getRawParameterValue(p + PrKey::mask) > PrValue::boolThread);

        params.opl3.op[op].isOplMode = true;
        params.opl3.op[op].regEnable = true;
        params.opl3.op[op].rar = (int)*apvts.getRawParameterValue(p + PrKey::rgAr);
        params.opl3.op[op].rdr = (int)*apvts.getRawParameterValue(p + PrKey::rgDr);
        params.opl3.op[op].rsl = (int)*apvts.getRawParameterValue(p + PrKey::rgSl);
        params.opl3.op[op].rrr = (int)*apvts.getRawParameterValue(p + PrKey::rgRr);
        params.opl3.op[op].rtl = (int)*apvts.getRawParameterValue(p + PrKey::rgTl);
    }
}
