...
void OpnaOperator::prepare(OpnaLfo* pOpnaLfo)
{
	this->opnaLfo = pOpnaLfo;
}

void OpnaOperator::setParameters(const OpnaOpParams& params, float feedback, N88LfoParams lfoParams)
{
	ampEnv.setParameters(params.ampEnv);
	n88Env.setParameters(params.regEnv);
	ssgEnv.setParams(params.ssgEnv);
  	fixMode.setParameters(params.fixMode);
    n88Lfo.setParameters(lfoParams);
	globalLfo.setParameters(params.globalLfo);
...
void OpnaOperator::noteOn(float frequency, float velocity, int noteNumber)
{
    ...
    phaseDelta = (finalFreq * 2.0 * juce::MathConstants<float>::pi) / sampleRate;
    ...
    targetLevel = velocity * tlGain * kslAttenuation;
    state = State::Attack;
...
void OpnaOperator::getSample(float& output, float modulator, float modWheel)
{
    if (state == State::Idle) { output = 0.0f; return; }

    updateEnvelopeState();
    float envVal = currentLevel;

    if (ssgEnv.isActive()) {
		envVal = ssgEnv.updatePhase(envVal, sampleRate);
    }

    // Sync Delay 更新
    if (m_lfoDelayCounter > 0.0f) {
        m_lfoDelayCounter -= 1.0f / (float)sampleRate;
    ...
    if (globalLfo.pm.enable || globalLfo.am.enable) {
        if (m_lfoDelayCounter > 0.0f) {
        }
        else {
            // LFO FREQ の選択 (0〜7)
    ...
    if (feedback > 0.0f) {
        float fbScale = std::pow(2.0f, feedback) / 64.0f;
...
// 6: +0.25%
// 7: +0.45%
const std::array<float, 8> OpnaOperator::dtScales = { 0.0f, -0.0045f, -0.0025f, -0.001f, 0.0f, 0.001f, 0.0025f, 0.0045f };
...
// 2: x1.58 (780 cent up)
// 3: x1.78 (950 cent up)
const std::array<float, 4> OpnaOperator::dt2Scales = { 1.0f, 1.414f, 1.581f, 1.781f };
...
float OpnaOperator::calcWaveform(double phase, int wave)
{
...
void OpnaOperator::updateEnvelopeState()
{
...
void OpnaOperator::updateIncrementsWithKeyScale()
{
