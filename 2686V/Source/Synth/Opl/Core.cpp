...
void OplCore::setParameters(const SynthParams& params) {
    m_alg.idx = params.opl.algorithm; // Range: 0-27

    if (m_rateIndex != params.opl.rateIndex) {
        m_rateIndex = params.opl.rateIndex;
        double target = getTargetRate(m_rateIndex);
        for (auto& op : m_operators) op.setSampleRate(target);
    }

    m_quantizeSteps = getTargetBitDepth(params.opl.bitDepth);

    for (int i = 0; i < 2; ++i) {
        float fb = (i == 0) ? params.opl.feedback0 : 0.0f; // OP1のみFeedback
...
const std::array<OplCore::BaseAlgRouting, 8> OplCore::baseRoutings = { {
...
// -----------------------------------------------------------
const std::array<OplCore::LfoCalculator, 5> OplCore::lfoStrategies = { {
        // 0: Sine
        [](double phase, float /*noise*/) -> OplCore::LfoResult {
...
// -----------------------------------------------------------
const std::array<OplCore::LfoCalculator, 4> OplCore::lfoN88Strategies = { {
        // 0: Saw Up
        [](double phase, float /*noise*/) -> OplCore::LfoResult {
...
// -----------------------------------------------------------
const std::array<OplCore::LfoCalculator, 6> OplCore::lfoN8886Strategies = { {
        // 0: Saw Up
        [](double phase, float /*noise*/) -> OplCore::LfoResult {
            return { (float)(phase < 0.5 ? phase * 2.0 - 1.0 : phase * 2.0 - 2.0), (float)(phase) };
        },
