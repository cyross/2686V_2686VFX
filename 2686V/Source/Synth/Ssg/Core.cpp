...
void SsgCore::setSampleRate(double sampleRate) { sampleCore.prepare(sampleRate); m_adsr.updateIncrements(sampleRate); }
...
void SsgCore::setParameters(const SynthParams& params)
{
	...
	m_adsr.setParameters(params.ssg.adsr, params.ssg.bypass);
    ...
    m_quantizeSteps = SynthHelpers::getTargetBitDepth(params.ssg.bitDepth);
...
float SsgCore::getSample()
{
    ...
    {
        if (m_adsr.isReleasing()) {
			sampleCore.currentLevel = 0.0f;
            m_adsr.idle();

            return 0.0f;
        }

        sampleCore.currentLevel = 1.0f;
    }
    else
    {
        sampleCore.currentLevel = m_adsr.process(sampleCore.currentLevel);
    }

    sampleCore.preProcess();
    ...
    while (sampleCore.isActive())
    {
        ...
        // ==========================================
        double lfoInc = m_lfoFreq / sampleCore.targetSampleRate;
        m_lfoPhase += lfoInc;
        ...
        if (m_waveform == 1 && !m_triKeyTrack) {
            phaseInc = (m_triFreq / (float)sampleCore.targetSampleRate) * freqMult;
        }
        else {
        ...
        // ==========================================
        float hwEnvDelta = m_envFreq / (float)sampleCore.targetSampleRate;
