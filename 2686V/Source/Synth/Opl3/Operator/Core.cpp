...
// 6: +0.25%
// 7: +0.45%
const std::array<float, 8> Opl3Operator::dtScales = { 0.0f, -0.0045f, -0.0025f, -0.001f, 0.0f, 0.001f, 0.0025f, 0.0045f };
...
// 2: x1.58 (780 cent up)
// 3: x1.78 (950 cent up)
const std::array<float, 4> Opl3Operator::dt2Scales = { 1.0f, 1.414f, 1.581f, 1.781f };

const std::array<Opl3Operator::SsgWaveCalculator, 16> Opl3Operator::ssgWaveStrategies = { {
    [](double p) { // 00: normal
        return 1.0f;
    },
    [](double p) { // 01
...
void Opl3Operator::updateEnvelopeState()
{
...
void Opl3Operator::updateIncrementsWithKeyScale()
{
    ...
    if (params.regEnable)
    {
        ...
        int keyRate = (octave * 2) + ((noteOffset > 7) ? 1 : 0);
        ksrValue = params.keyScale > 0 ? keyRate : (keyRate >> 2);
