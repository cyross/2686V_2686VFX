...
void Opl3Core::setParameters(const SynthParams& params) {
    m_alg.idx = params.opl3.algorithm;

    if (m_rateIndex != params.opl3.rateIndex) {
        m_rateIndex = params.opl3.rateIndex;
        double target = getTargetRate(m_rateIndex);
        for (auto& op : m_operators) op.setSampleRate(target);
    }
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (i == 0) fb = params.opl3.feedback0;
...
const std::array<Opl3Core::BaseAlgRouting, 8> Opl3Core::baseRoutings = { {
...
// -----------------------------------------------------------
const std::array<Opl3Core::LfoCalculator, 5> Opl3Core::lfoStrategies = { {
        // 0: Sine
        [](double phase, float /*noise*/) -> Opl3Core::LfoResult {
...
// -----------------------------------------------------------
const std::array<Opl3Core::LfoCalculator, 4> Opl3Core::lfoN88Strategies = { {
        // 0: Saw Up
        [](double phase, float /*noise*/) -> Opl3Core::LfoResult {
...
// -----------------------------------------------------------
const std::array<Opl3Core::LfoCalculator, 6> Opl3Core::lfoN8886Strategies = { {
        // 0: Saw Up
        [](double phase, float /*noise*/) -> Opl3Core::LfoResult {
            return { (float)(phase < 0.5 ? phase * 2.0 - 1.0 : phase * 2.0 - 2.0), (float)(phase) };
        },
