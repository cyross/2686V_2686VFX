...
void OpmCore::setParameters(const SynthParams& params) {
    ...
    if (m_rateIndex != params.opm.rateIndex) {
        m_rateIndex = params.opm.rateIndex;
        double target = getTargetRate(m_rateIndex);
        for (auto& op : m_operators) op.setSampleRate(target);
        updateNoiseDelta(target);
    }
...
const std::array<OpmCore::BaseAlgRouting, 8> OpmCore::baseRoutings = { {
...
// -----------------------------------------------------------
const std::array<OpmCore::LfoCalculator, 5> OpmCore::lfoStrategies = { {
        // 0: Sine
        [](double phase, float /*noise*/) -> OpmCore::LfoResult {
...
// -----------------------------------------------------------
const std::array<OpmCore::LfoCalculator, 4> OpmCore::lfoN88Strategies = { {
        // 0: Saw Up
        [](double phase, float /*noise*/) -> OpmCore::LfoResult {
...
// -----------------------------------------------------------
const std::array<OpmCore::LfoCalculator, 6> OpmCore::lfoN8886Strategies = { {
        // 0: Saw Up
        [](double phase, float /*noise*/) -> OpmCore::LfoResult {
            return { (float)(phase < 0.5 ? phase * 2.0 - 1.0 : phase * 2.0 - 2.0), (float)(phase) };
        },
