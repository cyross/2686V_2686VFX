...
// 6: +0.25%
// 7: +0.45%
const std::array<float, 8> OpmOperator::dtScales = { 0.0f, -0.0045f, -0.0025f, -0.001f, 0.0f, 0.001f, 0.0025f, 0.0045f };
...
// 2: x1.58 (780 cent up)
// 3: x1.78 (950 cent up)
const std::array<float, 4> OpmOperator::dt2Scales = { 1.0f, 1.414f, 1.581f, 1.781f };

const std::array<OpmOperator::SsgWaveCalculator, 16> OpmOperator::ssgWaveStrategies = { {
    [](double p) { // 00: normal
        return 1.0f;
    },
    [](double p) { // 01
...
float OpmOperator::calcWaveform(double phase, int wave)
{
...
void OpmOperator::updateEnvelopeState()
{
    if (state == State::Attack) {
        ...
        // ====================================================================
        if (params.isOplMode && !params.egType) {
...
void OpmOperator::updateIncrementsWithKeyScale()
{
