...
public:
    ...
    float getSample() {
        ...
        // Wave: 0:Saw, 1:Square, 2:Tri, 3:Noise
        switch (m_lfoWave) {
            case 0: // Sawtooth (Down)
                lfoVal = (float)(1.0 - m_lfoPhase * 2.0);
                break;
            case 1: // Square
                lfoVal = (m_lfoPhase < 0.5) ? 1.0f : -1.0f;
                break;
            case 2: // Triangle
        ...
        // PMS/AMS depth calculation (Simplified)
        float lfoAmpMod = 1.0f;
        if (m_ams > 0) {
            // OPM AMS Depth approx
            float depths[] = { 0.0f, 0.1f, 0.3f, 0.7f }; // 簡易値
