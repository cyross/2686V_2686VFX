...
// -----------------------------------------------------------
const std::array<Opzx7LfoCore::Opzx7LfoCalculator, 8> Opzx7LfoCore::lfoPgStrategies = { {
        // 0: Sine
...
// -----------------------------------------------------------
const std::array<Opzx7LfoCore::Opzx7LfoCalculator, 8> Opzx7LfoCore::lfoEgStrategies = { {
        // 0: Sine
        [](double phase, float /*noise*/)-> float {
...
void Opzx7LfoCore::setParameters(int syncDelay, bool pm, bool am, float pmFreq, float amFreq, int pgIndex, int egIndex, float pms, float pmd, float ams, float amd, float amSmoothRate)
{
    ...
    this->m_pgWaveIndex = std::clamp(pgIndex, 0, 7);
    this->m_isOneshotPg = this->m_pgWaveIndex == 6 || this->m_pgWaveIndex == 7;
    ...
    this->m_egWaveIndex = std::clamp(egIndex, 0, 7);
    this->m_isOneshotEg = this->m_egWaveIndex == 6 || this->m_egWaveIndex == 7;
    this->ams = ams;
    this->amd = amd;
...
void Opzx7LfoCore::noteOn()
{
    ...
    if (this->m_sdParam == 0) {
        ...
        if (this->m_isOneshotPg || this->m_isOneshotEg) {
            this->m_pmPhase = 0.0;
            this->m_amPhase = 0.0;
        }
    }
...
void Opzx7LfoCore::getSample()
{
    ...
    if (this->pmEnable || this->amEnable) {
        if (this->m_sdCounter <= 0.0f) {
            ...
            if (this->m_isOneshotEg && this->m_amCycleCount > 0) amVal = 0.0f;
        }
    }
