...
OplAdsr::OplAdsr()
{
    ...
    this->setParameterFunctions = std::array<std::function<void(const OplAdsrParams&)>, 2>{
		[this](const OplAdsrParams& params) {
            // サステインレベル (SL) の計算
            if (this->sl == 15) {
    ...
    this->noteOnFunctions = std::array<std::function<float(float, int)>, 2>{
		[this](float velocity, int noteNumber) {
        ...
        std::array<std::function<float(float)>, 5>{
            ...
            [this](float newLevel) { // Attack
                ...
                if (this->m_phaseProgress >= 1.0f) {
					...
					outLevel = 1.0f; // 最後のサンプルは必ず1.0fを返す
                }
...
float OplAdsr::noteOn(float velocity, int noteNumber) {
    ...
    return this->noteOnFunctions[this->m_curveCore == nullptr ? 0 : this->m_curveCore->index](velocity, noteNumber);
}
