...
// OPZX7 WS (オペレータの波形)
// ============================================================================
std::vector<float> WavePreviewSource::opzx7Ws(int shapeIndex, std::vector<float>& wt, std::vector<float>& wt2, std::vector<float>& pcm,
    float offsetMs, float ratio, double sampleRate)
{
    // calcWaveform は波形メモリと PCM のときだけメンバを見る。それ以外は
    // 位相と番号だけで決まるので、オペレータを 1 つ組んで呼ぶだけでよい。
    Opzx7Operator op;

    // PCM の切り出しはサンプリングレートを見るので、先に渡しておく
    op.prepare(0, sampleRate);

    op.m_params.waveSelect = shapeIndex;
    op.m_params.pcm.offset = offsetMs;
    op.m_params.pcm.ratio = ratio;

    op.setWtBuffer(wt.empty() ? nullptr : &wt);
    op.setWt2Buffer(wt2.empty() ? nullptr : &wt2);
    op.setPcmBuffer(pcm.empty() ? nullptr : &pcm);
    ...
    // 位相はサイクル単位。1 周ぶん描く。
    for (int i = 0; i < points; ++i) {
        out.push_back(op.calcWaveform((double)i / (double)points, shapeIndex));
    }
...
namespace
{
    std::vector<float> runOpm(int shapeIndex, bool isAm, float smoothRate)
    {
        // 3 は毎周期引き直すノイズ。それ以外に段のある波形は無い。
        const LfoWindow w = makeLfoWindow(shapeIndex == 3);
        ...
        p.pm = true;
        p.am = true;
        p.pgIndex = shapeIndex;
        p.egIndex = shapeIndex;
        p.pmFreq = previewLfoFreq;
        p.amFreq = previewLfoFreq;
        p.amSmoothRate = smoothRate;
...
std::vector<float> WavePreviewSource::opmLfoPm(int shapeIndex)
{
    return runOpm(shapeIndex, false, 1.0f);
}

std::vector<float> WavePreviewSource::opmLfoAm(int shapeIndex, float smoothRate)
{
    return runOpm(shapeIndex, true, smoothRate);
}
