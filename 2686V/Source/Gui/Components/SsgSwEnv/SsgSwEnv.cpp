...
void GuiComponentSsgSwEnv::setupComponent(juce::Component& parent, const juce::String& code, int &tabOrder, const juce::String& flagKey, const juce::String& flagText, bool isEnable)
{
    ...
    steps.setup({ .parent = parent, .id = code + "_SSGSW_STEPS", .title = "Steps", .isReset = true });
    ...
    loopTo.setup({ .parent = parent, .id = code + "_SSGSW_LOOPTO", .title = "Loop To", .isReset = true });
    ...
    loopCount.setup({ .parent = parent, .id = code + "_SSGSW_LOOPCNT", .title = "Loop Cnt", .isReset = true });
    ...
    startLevel.setup({ .parent = parent, .id = code + "_SSGSW_STL", .title = "Stl", .isReset = true });
    startLevel.setWantsKeyboardFocus(true);
    startLevel.setExplicitFocusOrder(++tabOrder);

    r1.setup({ .parent = parent, .id = code + "_SSGSW_R1", .title = "R1", .isReset = true });
    r1.setWantsKeyboardFocus(true);
    r1.setExplicitFocusOrder(++tabOrder);

    l1.setup({ .parent = parent, .id = code + "_SSGSW_L1", .title = "L1", .isReset = true });
    l1.setWantsKeyboardFocus(true);
    l1.setExplicitFocusOrder(++tabOrder);

    r2.setup({ .parent = parent, .id = code + "_SSGSW_R2", .title = "R2", .isReset = true });
    r2.setWantsKeyboardFocus(true);
    r2.setExplicitFocusOrder(++tabOrder);

    l2.setup({ .parent = parent, .id = code + "_SSGSW_L2", .title = "L2", .isReset = true });
    l2.setWantsKeyboardFocus(true);
    l2.setExplicitFocusOrder(++tabOrder);

    r3.setup({ .parent = parent, .id = code + "_SSGSW_R3", .title = "R3", .isReset = true });
    r3.setWantsKeyboardFocus(true);
    r3.setExplicitFocusOrder(++tabOrder);

    l3.setup({ .parent = parent, .id = code + "_SSGSW_L3", .title = "L3", .isReset = true });
    l3.setWantsKeyboardFocus(true);
    l3.setExplicitFocusOrder(++tabOrder);

    r4.setup({ .parent = parent, .id = code + "_SSGSW_R4", .title = "R4", .isReset = true });
    r4.setWantsKeyboardFocus(true);
    r4.setExplicitFocusOrder(++tabOrder);

    l4.setup({ .parent = parent, .id = code + "_SSGSW_L4", .title = "L4", .isReset = true });
    l4.setWantsKeyboardFocus(true);
    l4.setExplicitFocusOrder(++tabOrder);

    r5.setup({ .parent = parent, .id = code + "_SSGSW_R5", .title = "R5", .isReset = true });
    r5.setWantsKeyboardFocus(true);
    r5.setExplicitFocusOrder(++tabOrder);

    l5.setup({ .parent = parent, .id = code + "_SSGSW_L5", .title = "L5", .isReset = true });
    l5.setWantsKeyboardFocus(true);
    l5.setExplicitFocusOrder(++tabOrder);

    r6.setup({ .parent = parent, .id = code + "_SSGSW_R6", .title = "R6", .isReset = true });
    r6.setWantsKeyboardFocus(true);
    r6.setExplicitFocusOrder(++tabOrder);

    l6.setup({ .parent = parent, .id = code + "_SSGSW_L6", .title = "L6", .isReset = true });
...
void GuiComponentSsgSwEnv::layoutComponent(juce::Rectangle<int>& rect)
{
    ...
    if (visible)
    {
        ...
        layoutMain({ .mainRect = rect, .label = &steps.label, .component = &steps });
        layoutMain({ .mainRect = rect, .component = &loop });
        layoutMain({ .mainRect = rect, .label = &loopTo.label, .component = &loopTo });
        layoutMain({ .mainRect = rect, .label = &loopCount.label, .component = &loopCount });
        layoutMain({ .mainRect = rect, .label = &startLevel.label, .component = &startLevel });
        layoutMain({ .mainRect = rect, .label = &r1.label, .component = &r1 });
        layoutMain({ .mainRect = rect, .label = &l1.label, .component = &l1 });
        layoutMain({ .mainRect = rect, .label = &r2.label, .component = &r2 });
        layoutMain({ .mainRect = rect, .label = &l2.label, .component = &l2 });
        layoutMain({ .mainRect = rect, .label = &r3.label, .component = &r3 });
        layoutMain({ .mainRect = rect, .label = &l3.label, .component = &l3 });
        layoutMain({ .mainRect = rect, .label = &r4.label, .component = &r4 });
        layoutMain({ .mainRect = rect, .label = &l4.label, .component = &l4 });
        layoutMain({ .mainRect = rect, .label = &r5.label, .component = &r5 });
        layoutMain({ .mainRect = rect, .label = &l5.label, .component = &l5 });
        layoutMain({ .mainRect = rect, .label = &r6.label, .component = &r6 });
        layoutMain({ .mainRect = rect, .label = &l6.label, .component = &l6 });
    }
}
...
void GuiComponentSsgSwEnv::layoutComponentRow(juce::Rectangle<int>& rect)
{
    ...
    if (visible)
    {
        ...
        layoutRow({ .rowRect = rect, .label = &steps.label, .component = &steps });
        layoutRow({ .rowRect = rect, .component = &loop });
        layoutRow({ .rowRect = rect, .label = &loopTo.label, .component = &loopTo });
        layoutRow({ .rowRect = rect, .label = &loopCount.label, .component = &loopCount });
        layoutRow({ .rowRect = rect, .label = &startLevel.label, .component = &startLevel });
        layoutRow({ .rowRect = rect, .label = &r1.label, .component = &r1 });
        layoutRow({ .rowRect = rect, .label = &l1.label, .component = &l1 });
        layoutRow({ .rowRect = rect, .label = &r2.label, .component = &r2 });
        layoutRow({ .rowRect = rect, .label = &l2.label, .component = &l2 });
        layoutRow({ .rowRect = rect, .label = &r3.label, .component = &r3 });
        layoutRow({ .rowRect = rect, .label = &l3.label, .component = &l3 });
        layoutRow({ .rowRect = rect, .label = &r4.label, .component = &r4 });
        layoutRow({ .rowRect = rect, .label = &l4.label, .component = &l4 });
        layoutRow({ .rowRect = rect, .label = &r5.label, .component = &r5 });
        layoutRow({ .rowRect = rect, .label = &l5.label, .component = &l5 });
        layoutRow({ .rowRect = rect, .label = &r6.label, .component = &r6 });
        layoutRow({ .rowRect = rect, .label = &l6.label, .component = &l6 });
    }
}
