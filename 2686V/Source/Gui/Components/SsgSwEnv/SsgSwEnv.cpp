...
float GuiComponentSsgSwEnv::getStepValue(const juce::String& key) const
{
...
void GuiComponentSsgSwEnv::setStepValue(const juce::String& key, float value)
{
...
void GuiComponentSsgSwEnv::rebindRate()
{
...
void GuiComponentSsgSwEnv::rebindLevel()
{
...
void GuiComponentSsgSwEnv::refreshStepValues()
{
...
void GuiComponentSsgSwEnv::importParams()
{
    ...
    ctx.editor.openParamBrowser(ctx.audioProcessor.defaultSsgSwEnvParamDir,
        { EditorGuiText::ParamBrowser::kindSsgSwEnv },
        [this](const juce::File& file) { applyParamsFile(file); });
}

// ブラウザから直に渡せるよう、ダイアログを出すところと
// 読んで反映するところを分けてある。
void GuiComponentSsgSwEnv::applyParamsFile(const juce::File& file)
{
...
void GuiComponentSsgSwEnv::exportParams()
{
    ...
    ctx.editor.openParamBrowserToSave(ctx.audioProcessor.defaultSsgSwEnvParamDir,
        { EditorGuiText::ParamBrowser::kindSsgSwEnv }, Io::Extension::SsgSwEnvParam,
        [this](const juce::File& file) { writeParamsFile(file); });
}

// ブラウザから直に渡せるよう、書き出す先を決めるところと
// 実際に書くところを分けてある。
void GuiComponentSsgSwEnv::writeParamsFile(const juce::File& file)
{
