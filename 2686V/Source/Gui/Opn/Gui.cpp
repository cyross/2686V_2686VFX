...
void GuiOpn::applyMmlString(const juce::String& mml, int opIndex)
{
    ...
    std::map<juce::String, std::function<void(int)>> actionMap = {
        ...
        { mmlPrefixDr,   [&](int v) {
            rgMode ? rgDr[opIndex].setValue(RegisterConverter::convertFmRg31(v), juce::sendNotification)
                   : dr[opIndex].setValue(RegisterConverter::convertFmDr(v), juce::sendNotification);
        }},
        { mmlPrefixSr,   [&](int v) {
            rgMode ? rgSr[opIndex].setValue(RegisterConverter::convertFmRg31(v), juce::sendNotification)
                   : sr[opIndex].setValue(RegisterConverter::convertFmSr(v), juce::sendNotification);
        }},
        { mmlPrefixSl,   [&](int v) {
            rgMode ? rgSl[opIndex].setValue(RegisterConverter::convertFmRg15(v), juce::sendNotification)
                   : sl[opIndex].setValue(RegisterConverter::convertFmSl(v), juce::sendNotification);
        }},
        { mmlPrefixRr,   [&](int v) {
