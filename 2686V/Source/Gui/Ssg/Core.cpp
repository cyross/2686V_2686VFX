...
void GuiSsg::setup()
{
    ...
    mul.setup({ .parent = *this, .id = code + SsgPrKey::mul, .title = SsgGuiText::Ssg::Detune::Mul, .items = multems, .isReset = true });
    mul.setWantsKeyboardFocus(true);
    mul.setExplicitFocusOrder(++tabOrder);

    mulRatio.setup({ .parent = *this, .id = code + SsgPrKey::mulRatio, .title = SsgGuiText::Ssg::Detune::MulRatio, .isReset = true });
...
void GuiSsg::updateGraph()
{
    ...
    // =============================================================
        ...
        float sl = slVal / slMax; // 15=0.0, 0=1.0
