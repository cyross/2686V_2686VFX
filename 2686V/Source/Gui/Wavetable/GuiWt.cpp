...
void GuiWt::setup()
{
    ...
    modShapeSelector.setup({ .parent = mainGroup.contentCanvas, .id = code + CPK::WtMod::shape, .title = WtGuiText::Wt::Mod::shape, .items = wtModShapeItems, .isReset = true, .isResized = true });
    modShapeSelector.setWantsKeyboardFocus(true);
    modShapeSelector.setExplicitFocusOrder(++tabOrder);

    modWaveWtBtn.setup({ .parent = mainGroup.contentCanvas, .title = WtGuiText::Wt::Mod::waveWt, .bgColor = juce::Colours::darkgrey.brighter(0.2f), .isReset = false, .isResized = true });
    ...
    modWaveWt2Btn.setup({ .parent = mainGroup.contentCanvas, .title = WtGuiText::Wt::Mod::waveWt2, .bgColor = juce::Colours::darkgrey.brighter(0.2f), .isReset = false, .isResized = true });
    ...
    modWaveClearBtn.setup({ .parent = mainGroup.contentCanvas, .title = WtGuiText::Wt::Mod::waveClear, .textColor = juce::Colours::white, .bgColor = juce::Colours::darkred.withAlpha(0.7f), .isReset = false, .isResized = true });
    ...
    if (ctx.audioProcessor.wtModWavePath.isNotEmpty()) {
        updateModWaveFileName(juce::File(ctx.audioProcessor.wtModWavePath).getFileName());
    }

    modWaveSmoothBtn.setup({ .parent = mainGroup.contentCanvas, .id = code + CPK::WtMod::waveSmooth, .title = WtGuiText::Wt::Mod::waveSmooth, .isReset = true, .isResized = true });
...
void GuiWt::importModWave(bool isWt2)
{
            ...
            juce::Timer::callAfterDelay(50, [this, isWt2, file]()
                {
                    ...
                    ctx.audioProcessor.wtModWavePath = file.getFullPathName();
                });
        }
    );
}
...
bool GuiWt::applyModWaveFile(const juce::File& file, bool isWt2)
{
...
void GuiWt::clearModWave()
{
    ...
    ctx.audioProcessor.wtModWavePath = juce::String();

    updateModWaveFileName(Io::empty);
}

void GuiWt::updateModWaveFileName(const juce::String& fileName)
{
