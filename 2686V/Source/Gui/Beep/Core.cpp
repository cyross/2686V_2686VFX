...
void GuiBeep::layout(juce::Rectangle<int> content) {
    ...
    auto graphAndBtnArea = mRect.removeFromBottom(150);
...
void GuiBeep::setupGraph()
{
    ...
    dr.onValueChange = repaintGraph;
    sl.onValueChange = repaintGraph;
...
void GuiBeep::layoutGraph(juce::Rectangle<int>& rect)
{
    ...
    graphBtnSsg.setBounds(btnArea.reduced(2, 0));
...
void GuiBeep::updateGraph()
{
    ...
    // =============================================================
        ...
        std::array<juce::Slider*, 7> rSl = { nullptr, &ssgSwR1Slider, &ssgSwR2Slider, &ssgSwR3Slider, &ssgSwR4Slider, &ssgSwR5Slider, &ssgSwR6Slider };
        std::array<juce::Slider*, 7> lSl = { &ssgSwStartLevelSlider, &ssgSwL1Slider, &ssgSwL2Slider, &ssgSwL3Slider, &ssgSwL4Slider, &ssgSwL5Slider, &ssgSwL6Slider };
        ...
        graph.setEnvelope(GuiEnvelopeGraph::EnvType::SsgSw, "SSG SW Env", phases);
    }
    // =============================================================
    // Amp Env
    // =============================================================
    else {
        float arMax = (float)ar.getMaximum();
        float drMax = (float)dr.getMaximum();
        float slMax = (float)sl.getMaximum();
        float rrMax = (float)rr.getMaximum();

        float arVal = (float)ar.getValue();
        float drVal = (float)dr.getValue();
        float slVal = (float)sl.getValue();
        float rrVal = (float)rr.getValue();
