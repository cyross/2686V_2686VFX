...
private:
    ...
    enum TabIndex
    {
        tabWt = 0, tabWt2, tabWtPlus, tabCurve, tabPreset, tabSettings, tabColors, tabAbout,
    ...
    // 後から作るタブを、作ってあるぶんだけ捨てる。
    void resetLazyTabs();

    // 言語が変わったので、画面の文字列を入れ直す。
    //
    // 部品はタイトルを setup のときに受け取り、その後は持たない。生きた
    // まま題を付け替える口を足すと、部品の種類 × 画面の数だけ書き漏らし
    // が出る。タブごと捨てて組み直すほうが確実なので、そうしている。
