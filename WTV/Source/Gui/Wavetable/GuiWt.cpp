...
namespace
{
	...
	// ファイルの中身を見分ける印
	const Io::ParamFormat wtFormat{ "wt", 1 };
...
void GuiWt::layout(juce::Rectangle<int> content)
{
    ...
    const bool pitchOpen = layoutMajorCategory(pitchMajorCat, mRect, true);
...
// 実際の読み書きを分けてある。
void GuiWt::applyWavetableFile(const juce::File& file)
{
...
// ブラウザから直に渡せるよう、ダイアログを出すところと
// 読んで反映するところを分けてある。
void GuiWt::applyQualityParamFile(const juce::File& file)
{
    if (!file.existsAsFile()) return;
...
// 実際に書くところを分けてある。
void GuiWt::writeQualityParamFile(const juce::File& file)
{
    if (file == juce::File{}) return;
...
void GuiWt::applyChParamFile(const juce::File& file) {
    ...
    auto reader = Io::ParamReader::open(file, wtFormat);
    ...
    auto customValues = reader->getFloatArray("customWave");

    for (auto& value : customValues) value = std::clamp(value, -1.0f, 1.0f);
...
// 当時の互換の工夫もそのまま残る。
void GuiWt::setImportingChParams(juce::StringArray& lines, int& index) {
	// Level
...
// 3.0.0 より前の形式を読む。移行のときに当時の読み手ごと書き換えて
// しまったので、履歴から戻したもの。
void GuiWt::setImportingQualityParams(juce::StringArray& lines, int& index) {
...
// 書き出す中身。エクスポートと変換の両方から使う。
void GuiWt::writeQualityParams(Io::ParamWriter& writer) {
...
void GuiWt::openEnabledCategories()
{
    ...
    if (anyCategoryEnabled(ampEnvComponent, ampModComponent, ssgHwEnv, ssgSwEnvComponent, ssgSwEnv11Component)) ampMajorCat.setDetailVisible(true);
    if (anyCategoryEnabled(modComponent, pitchEnvComponent, ssgHwPEnv, ssgSwPEnv11Component)) pitchMajorCat.setDetailVisible(true);
}

void GuiWt::closeBypassedCategories()
{
    ...
    if (allCategoriesBypassed(ampEnvComponent, ampModComponent, ssgHwEnv, ssgSwEnvComponent, ssgSwEnv11Component)) ampMajorCat.setDetailVisible(false);
    if (allCategoriesBypassed(modComponent, pitchEnvComponent, ssgHwPEnv, ssgSwPEnv11Component)) pitchMajorCat.setDetailVisible(false);
}
