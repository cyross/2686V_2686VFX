...
namespace PresetValue
{
	...
	namespace File
	{
		...
		static inline const juce::String glob = "*.pulsev.json;*.pulsev.yaml;*.xml";

		// 書き出すときの拡張子。選ばれている形に合わせる。
		static inline juce::String extension()
		{
			return ".pulsev." + Io::fileFormatExtension();
		}
		static inline const juce::String def = "Untitled";

		namespace Name
		{
			static inline const juce::String initial = "init_preset_pulsev.xml";

			// お気に入りと履歴の置き場。プラグインごとに分ける。
			// プリセットの置き場も拡張子もプラグインで違うため。
			static inline const juce::String library = "PULSEV.library";
		}
