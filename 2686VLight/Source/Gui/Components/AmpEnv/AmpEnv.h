...
public:
    ...
    bool isCategoryBypassed() const { return bypass.getToggleState(); }

    void setCategoryBypassed(bool bypassed) {
        bypass.setToggleState(bypassed, juce::sendNotification);
    }
    ...
    void importParams();
    // ブラウザから直に読ませるための入口。
    void applyParamFile(const juce::File& file);
    void exportParams();
    // ブラウザから直に渡せるようにした入口。
