...
{
    fileNameLabel.setText(Io::isFileName(fileName) ? fileName : Io::empty, juce::dontSendNotification);
...
// 読んで反映するところを分けてある。
void GuiAdpcm::applyToneNoiseParamFile(const juce::File& file)
{
...
// 実際に書くところを分けてある。
void GuiAdpcm::writeToneNoiseParamFile(const juce::File& file)
{
...
// ブラウザから直に渡せるよう、ダイアログを出すところと
// 読んで反映するところを分けてある。
void GuiAdpcm::applyQualityParamFile(const juce::File& file)
{
    if (!file.existsAsFile()) return;
...
// ブラウザから直に渡せるよう、書き出す先を決めるところと
// 実際に書くところを分けてある。
void GuiAdpcm::writeQualityParamFile(const juce::File& file)
{
    if (file == juce::File{}) return;
...
// ブラウザから直に渡せるよう、ダイアログを出すところと
// 読んで反映するところを分けてある。
void GuiAdpcm::applyPcmPlayParamFile(const juce::File& file)
{
    if (!file.existsAsFile()) return;
...
// ブラウザから直に渡せるよう、書き出す先を決めるところと
// 実際に書くところを分けてある。
void GuiAdpcm::writePcmPlayParamFile(const juce::File& file)
{
    if (file == juce::File{}) return;
