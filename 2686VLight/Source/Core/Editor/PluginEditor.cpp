...
namespace
{
    ...
    // ここは tools/gen_plugin.py も書き出す。音源を絞ると並びが変わるため。
    constexpr OscMode tabModes[] = {
        OscMode::OPNA, OscMode::OPN, OscMode::OPL, OscMode::OPL3,
        OscMode::OPM, OscMode::OPZX7, OscMode::SSG, OscMode::WAVETABLE,
        OscMode::WT2, OscMode::WTPLUS, OscMode::RHYTHM, OscMode::ADPCM,
        OscMode::ADPCMPLUS, OscMode::BEEP,
    };
...
GuiBase* AudioPlugin2686VEditor::tabGuiFor(int tabIndex)
{
    ...
    case tabOpl: return &oplGui.ref();
    case tabOpl3: return &opl3Gui.ref();
    case tabOpm: return &opmGui.ref();
    case tabOpn: return &opnGui.ref();
    case tabOpna: return &opnaGui.ref();
    case tabOpzx7: return &opzx7Gui.ref();
    case tabRhythm: return &rhythmGui.ref();
    case tabSsg: return &ssgGui.ref();
    case tabWt: return &wtGui.ref();
    case tabWt2: return &wt2Gui.ref();
...
int AudioPlugin2686VEditor::tabForCategory(const juce::String& category)
{
    ...
    if (category == "OPL") return tabOpl;
    if (category == "OPL3") return tabOpl3;
    if (category == "OPM") return tabOpm;
    if (category == "OPN") return tabOpn;
    if (category == "OPNA") return tabOpna;
    if (category == "OPZX7") return tabOpzx7;
    if (category == "RHYTHM") return tabRhythm;
    if (category == "SSG") return tabSsg;
    if (category == "WT") return tabWt;
    if (category == "WT2") return tabWt2;
